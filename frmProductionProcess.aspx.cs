...
{
    public partial class frmProductionProcess : System.Web.UI.Page
    {
        ...
        public void bindgrid(string sortExpression = null)
        {
            ...
            try
            {
                ...
                {
                    qry = " Select  A.cerpac_no, (A.forename+' '+A.surname)as Name, A.cerpac_file_no as FORMNO, A.CompanyName, (Case when A.cerpac_no= A.cerpac_file_no then 'Fresh Case' Else 'Renewal Case' End) as Status,ProducedBy   " +
                       " from vw_ApplicantDetails A LEFT OUTER JOIN vw_ApplicantDocument B ON A.cerpac_file_no=B.Form_No where isnull(A.IsAuthorized,'0')='1' and isnull(ISARCR,'0')='1' and isnull(IsProduced,'0')='0'   group by  A.CreatedOn, A.cerpac_no, (A.forename+' '+A.surname) , A.cerpac_file_no, A.CompanyName, (Case when A.cerpac_no= A.cerpac_file_no then 'Fresh Case' Else 'Renewal Case' End),ProducedBy " +
                       " order by " + Condition + "  ";
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    GridViewCard.DataSource = dt;
                    GridViewCard.DataBind();
                }
                else
                {
                    //lbl_total.Text = dt.Rows.Count.ToString();
                    GridViewCard.DataSource = null;
                    GridViewCard.DataBind();
                }
        ...
        protected void Page_PreRender(object sender, EventArgs e)
        {
            ...
            if (grdLevel3flag == "1")
                Position(GridViewCard, sortOrder, currentSortColumn);
            else
                Position(GridViewCard, sortOrder, currentSortColumn);
        }
        ...
        protected void Go_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                " from vw_ApplicantDetails A LEFT OUTER JOIN vw_ApplicantDocument B ON A.cerpac_file_no=B.Form_No where isnull(A.IsAuthorized,0)=1 and isnull(ISARCR,0)=1 and isnull(IsProduced,0)=0  and ( A.cerpac_no like '" + TextAppId.Text.ToString() + "%' or A.cerpac_file_no like '" + TextAppId.Text.ToString() + "%' ) " +
        ...
        protected void GridViewCard_PageIndexChanging(object sender, GridViewPageEventArgs e)
        {
            GridViewCard.PageIndex = e.NewPageIndex;
            this.bindgrid();
        }

        protected void GridViewCard_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            try
            {
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    string columnNameValue = rowView["Producedby"].ToString();

                    if (columnNameValue == dt_login_details.Rows[0]["Userid"].ToString() || columnNameValue == "")
                    {
                        e.Row.Attributes["onclick"] = Page.ClientScript.GetPostBackClientHyperlink(GridViewCard, "Select$" + e.Row.RowIndex);
        ...
        protected void GridViewCard_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            try
            {
                ...
                //GridViewRow row = GridViewCard.Rows[rowIndex];
        ...
        public void bindgrid_assign(string lblCerpacNo)
        {
            try
            {
                //Label lblCerpacNo = (Label)GridViewCard.SelectedRow.FindControl("lblCerpacNo");
                ...
                if (dt.Rows.Count > 0)
                {
                    if (dt.Rows[0]["ProducedBy"].ToString().Trim() != "" && dt.Rows[0]["ProducedBy"].ToString().Trim() != dt_login_details.Rows[0]["Userid"].ToString())
                    {
                        lblloginmsg.Attributes.Add("class", "active");
                        lblloginmsg.Attributes["style"] = "color:red; font-weight:bold;";
                    ...
                    string updateQuery = "UPDATE peoplechild " +
                                     "SET ProducedBy = @VerifiedBy, ProducedOn = GETDATE() " +
                                     "WHERE FormNo = @FormNo";
                    ...
                    txtIssuedate.Text = CommonFunctions.ConvertToDateStringFormat(dt.Rows[0]["cerpac_receipt_date"].ToString());

                    //DateTime cerpacExpiryDate = Convert.ToDateTime(dt.Rows[0]["cerpac_expiry_date"]);
                    //txtExpirydate.Text = cerpacReceiptDate.ToString("yyyy-MM-dd").Trim();
                    txtExpirydate.Text = CommonFunctions.ConvertToDateStringFormat(dt.Rows[0]["cerpac_expiry_date"].ToString());

                    // assuming your dt.Rows[0]["cerpac_receipt_date"] = "02/01/2026"

                    //string receiptDateStr = dt.Rows[0]["cerpac_receipt_date"].ToString();
                    //string expiryDateStr = dt.Rows[0]["cerpac_expiry_date"].ToString();

                    //DateTime cerpacReceiptDate = DateTime.Parse(receiptDateStr, new CultureInfo("en-GB"));
                    //txtIssuedate.Text = cerpacReceiptDate.ToString("yyyy-MM-dd");

                    //DateTime cerpacExpiryDate = DateTime.Parse(expiryDateStr, new CultureInfo("en-GB"));
                    //txtExpirydate.Text = cerpacExpiryDate.ToString("yyyy-MM-dd");
                    ...
                    string Qty4 = "Select A.ZoneNote, B.Username from peoplechild A, usermaster B where A.ConfirmedBy=B.UserID and A.Cerpacno ='" + lblCerpacNo.ToString().Trim() + "' " +
        ...
        protected void GridViewCard_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                Label lblCerpacNo = (Label)GridViewCard.SelectedRow.FindControl("lblCerpacNo");
                ...
                string Qty1 = "SELECT App_id, Title,people_id , Category_Type, tile1,forename,middle_name,surname,sex,cerpac_no,cerpac_file_no,cerpac_receipt_date , picture    ,cerpac_expiry_date,registration_area,registration_code,file_no,passport_no,nationality,passport_issue_date,passport_expiry_date " +
                    ",passport_issue_loc,passport_issue_by,date_of_birth,place_of_birth,nigeria_add_1,nigeria_add_2,nigeria_tel_no,abroad_add_1     ,abroad_add_2,abroad_tel_no,company,company_add_1,company_add_2,designation,employment_date,company_tel_no,company_fax_no " +
                    ",email,verid_template,ContactNo,userImage,IsVerified,IsAuthorized,IsProduced,VerifiedBy,AuthorizedBy,ProducedBy     ,ProducedOn,AuthorizedOn,VerifiedOn,IsRejected,RejectedBy,RejectedOn,RejectionReason,ZoneNote,AuthNote,ProdNote,QualNote " +
                    ",IsDependent,DesignationCode,DependentOn,ISARCR,ConfirmedBy,ConfirmedOn,Cerpac_ExpiryDt,Cerpac_IssuedDt,IsQual,QualBy,QualOn,CompanyName    , convert(varchar(10), date_system_modified ,103) as date_system_modified, AppType,PaymentMode1,MaritalStatus " +
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    // Format the DateTime to the desired string format
                    //txtExpirydate.Text = cerpacReceiptDate.ToString("yyyy-MM-dd").Trim();
                    //txtExpirydate.Text = string.Format("{0:yyyy-MM-dd}", dt.Rows[0]["cerpac_expiry_date"]).ToString().Trim();
                    //CommonFunctions.ConvertToDateStringFormat(dt.Rows[0]["cerpac_expiry_date"]).ToString().Trim();
                    /***********Personal Details************************/
                    lblTitle.Text = dt.Rows[0]["title"].ToString().Trim();
                    lblFristName.Text = dt.Rows[0]["forename"].ToString().Trim();
                    lblLastName.Text = dt.Rows[0]["surname"].ToString().Trim();
        ...
        protected void btnApproved_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (x == 0)
                {
                    ...
                    cmd.Parameters.AddWithValue("@prodnotes", Textarea1.Text.Trim());
        ...
        //private int CalDate()
        //{
        //    //string d1 = (CommonFunctions.ConvertToDateStringFormat(txtIssuedate.Text.ToString()));
        //    //string d2 = (CommonFunctions.ConvertToDateStringFormat(txtExpirydate.Text.ToString()));
        //    //string d3 = (d2 - d1).TotalDays.ToString();

        //    DateTime d1, d2;
        //    if (DateTime.TryParse(txtIssuedate.Text, out d1) && DateTime.TryParse(txtExpirydate.Text, out d2))
        //    {
        //        string d3 = (d2 - d1).TotalDays.ToString();
        ...
        private int CalDate()
        {
            ...
            if (!DateTime.TryParse(txtIssuedate.Text, out d1))
            {
                Response.Write("<script>alert('Invalid Issue Date')</script>");
                return 1;
            }

            if (!DateTime.TryParse(txtExpirydate.Text, out d2))
            {
            ...
            string cerpacPrefix = lblCerpacNo1.Text.ToUpper().Substring(0, 2);
            string companyName = lblCompanyName.Text.ToUpper();
            string designation = lblDesignation.Text.ToUpper();

            if ((cerpacPrefix == "AO" || cerpacPrefix == "CR" || cerpacPrefix == "AB") && totalDays >= 366)
            {
                Response.Write("<script>alert('Cerpac Expiry Date should be less than or equal to one year from issue date')</script>");
                return 1;
            }

            if (cerpacPrefix == "AR" && totalDays > 735)
            {
                if (companyName != "NIGER WIFE" && designation != "NIGER WIFE" && companyName != "SPECIAL IMMIGRATION STATUS" && designation != "SPECIAL IMMIGRATION STATUS")
                {
                    Response.Write("<script>alert('Cerpac Expiry Date for AR category should be less than or equal to two years from issue date')</script>");
                    return 1;
                }
        ...
        private void SendMail()
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                using (StreamReader reader = new StreamReader(HttpContext.Current.Server.MapPath("~/WebApp_Template.html")))
                {
                    {
        ...
        protected void btnReferred_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (x == 0)
                {
                    SqlCommand cmd = new SqlCommand();
                    cmd.Connection = con;
                    cmd.CommandText = "USP_CERPAC_CardCorrection";
                    cmd.CommandType = CommandType.StoredProcedure;
                    ...
                    cmd.Parameters.AddWithValue("@UserId", dt_login_details.Rows[0]["Userid"].ToString());
                    cmd.Parameters.AddWithValue("@prodnotes", Textarea1.Text.Trim());
                    cmd.Parameters.AddWithValue("@Flag", "CardRefer");
                    SqlParameter output = new SqlParameter("@SuccessId", SqlDbType.Int);
                    output.Direction = ParameterDirection.Output;
                    cmd.Parameters.Add(output);
                    con.Open();
                    ...
                    if (R != String.Empty)
                    {
                        ...
                        pnlmsg.Attributes.Add("style", "display:none;");
                        pnlmsgCorrection.Attributes.Add("style", "display:none;");
                        pnlmsgRefer.Attributes.Add("style", "display:none;");
                        pnlmsgRefer.Attributes.Add("style", "display:block;");
        ...
        protected void btnCorrection_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (x == 0)
                {
                    ...
                    cmd.Parameters.AddWithValue("@UserId", dt_login_details.Rows[0]["Userid"].ToString());
                    cmd.Parameters.AddWithValue("@prodnotes", Textarea1.Text.Trim());
                    cmd.Parameters.AddWithValue("@Flag", "CardCorrection");
                    SqlParameter output = new SqlParameter("@SuccessId", SqlDbType.Int);
                    output.Direction = ParameterDirection.Output;
                    cmd.Parameters.Add(output);
                    con.Open();
                    ...
                    if (R != String.Empty)
                    {
                        ...
                        pnlCard.Attributes.Add("style", "display:none;");
                        pnlmsg.Attributes.Add("style", "display:none;");
                        pnlmsgCorrection.Attributes.Add("style", "display:block;");
                        pnlmsgRefer.Attributes.Add("style", "display:none;");
        ...
        {
            BindComments();
            if (commentList.SelectedValue == "A")
            {
                btnReferred.Enabled = false;
                btnApproved.Enabled = true;
                btnCorrection.Enabled = false;
            }
            else if (commentList.SelectedValue == "P")
            {
                btnReferred.Enabled = true;
